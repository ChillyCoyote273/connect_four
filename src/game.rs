...
impl Game {
    ...
    fn point_to_board(x: f32, y: f32) -> (usize, usize) {
        (
            ((x / 125.0 + 3.5) as usize).min(6),
            ((y / 125.0 + 3.0) as usize).min(5),
        )
    }
    ...
    fn count_direction(
        ...
        for i in 1.. {
            ...
            if !(0..7).contains(&c)
                || !(0..6).contains(&r)
                || self.board[c as usize][r as usize] != color
            {
                return i as usize;
            }
        }
    ...
    pub fn get_bitboards(&self) -> (u64, u64) {
        ...
        for column in 0..7 {
            for row in 0..6 {
                if self.board[column][row] != Piece::Empty {
                    mask |= Bitboard::position_mask(column as u8, row as u8);
                    if self.board[column][row] == current_piece {
                        position |= Bitboard::position_mask(column as u8, row as u8);
                    }
                }
            }
        }
